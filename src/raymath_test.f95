PROGRAM raymath_test
  ...
  CALL test_vector_scalar_product(res)
...
SUBROUTINE test_vector_subtract(res)
  ...
  TYPE(vector)        :: u = vector(2.1, -5.3, 0)
...
SUBROUTINE test_vector_scalar_product(res)
  TYPE(test_result)   :: res
  TYPE(vector)        :: v = vector(3.0, 5.7, 2.75)
  TYPE(vector)        :: u = vector(2.1, -5.0, 1)
  REAL                :: e = 3.0*2.1 + 5.7*(-5.0) + 2.75
  REAL                :: r
  LOGICAL             :: failure
  r = vector_scalar_product(v, u)
  failure = assertTrue(res, ABS(r - e) < 0.00001)
  IF (failure) THEN
    PRINT *, "vector_scalar_product falhou. Esperado", e, " mas encontrado ", r
  END IF
  RETURN
END SUBROUTINE test_vector_scalar_product
